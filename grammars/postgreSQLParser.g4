...
alter_table_statement
    : FOREIGN? TABLE if_exists? ONLY? tn=schema_qualified_name MULTIPLY?(
...
create_type_statement
    : TYPE tname=schema_qualified_name (AS(
...
operator_option
    ...
    | (LEFTARG | RIGHTARG) EQUAL d_type=data_type
...
sequence_body
    : AS d_type=(SMALLINT | INTEGER | BIGINT)
...
create_schema_statement
    : SCHEMA if_not_exists? schname=identifier? (AUTHORIZATION user_name)?
    ;
...
create_database_statement
    : DATABASE dbname=identifier (WITH? create_database_option+)?
    ;
...
create_table_statement
    : ((GLOBAL | LOCAL)? (TEMPORARY | TEMP) | UNLOGGED)? TABLE if_not_exists? tn=schema_qualified_name
    define_table
    partition_by?
    (USING identifier)?
...
table_column_definition
    : cn=identifier dt=data_type define_foreign_options? collate_identifier? constraint_common*
    ;
...
constr_body
    : EXCLUDE (USING index_method=identifier)?
            ...
            index_parameters (where=WHERE exp=vex)?                                      #excludeconstr_body
    | (FOREIGN KEY fkey=names_in_parens)? REFERENCES reft=schema_qualified_name ref=names_in_parens?
        (MATCH (FULL | PARTIAL | SIMPLE) | ON (DELETE | UPDATE) action)*                 #fkeyconstr_body
    | CHECK LEFT_PAREN expression=vex RIGHT_PAREN (NO INHERIT)?                          #checkconstr_body
    | NOT? NULL                                                                          #notnullconstr_body
    | (UNIQUE | PRIMARY KEY) pkey=names_in_parens? index_parameters                       #pkeyconstr_body
    | DEFAULT default_expr=vex                                                           #defconstr_body
    | identity_body                                                                      #idconstr_body
    | GENERATED ALWAYS AS LEFT_PAREN vex RIGHT_PAREN STORED                              #genconstr_body
    ;
...
predefined_type
    : BIGINT
    | BIT VARYING? dlen=type_length?
    | BOOLEAN
    | DEC prec=precision_param?
    | DECIMAL prec=precision_param?
    | DOUBLE PRECISION
    | FLOAT prec=precision_param?
    | INT
    | INTEGER
    | INTERVAL interval_field? dlen=type_length?
    | NATIONAL? (CHARACTER | CHAR) VARYING? dlen=type_length?
    | NCHAR VARYING? dlen=type_length?
    | NUMERIC prec=precision_param?
    | REAL
    | SMALLINT
    | TIME dlen=type_length? ((WITH | WITHOUT) TIME ZONE)?
    | TIMESTAMP dlen=type_length? ((WITH | WITHOUT) TIME ZONE)?
    | VARCHAR dlen=type_length?
...
string_value_function
    : TRIM LEFT_PAREN (LEADING | TRAILING | BOTH)? (chars=vex FROM vstr=vex | FROM? vstr=vex (COMMA chars=vex)?) RIGHT_PAREN
