...
IdentifierStartChar
    ...
    [\u0100-\uD7FF\uE000-\uFFFF] {str(self._input.LA(-1)).isalpha()}?
    | // letters which require multiple UTF-16 code units
    [\uD800-\uDBFF] [\uDC00-\uDFFF] {(str(self._input.LA(-2)) + str(self._input.LA(-1))).encode('utf-16', 'surrogatepass').decode('utf-16').isalpha()}?
    ;
fragment
IdentifierChar
...
BeginDollarStringConstant
    : '$' Tag? '$' {_tags.appendleft(self.text)} -> pushMode(DollarQuotedStringMode)
    ;
...
EndDollarStringConstant
    : '$' Tag? '$' {self.text == _tags[0]}? {_tags.pop()} -> popMode
    ;
